...
def create_duplicate_dataset(tmp_path):
    dataframe = pd.DataFrame(
        {
            "age": [20, 21, 21, 22, 23, 24, 25, 26, 27, 28],
            "income": [
                30000,
                32000,
                32000,
    ...
    storage_key = "datasets/duplicate_test.csv"
    ...
    return DatasetReference(
        ...
        file_size=file_path.stat().st_size,
        mime_type="text/csv",
        checksum="duplicate-test-checksum",
    )
...
def test_prepare_removes_duplicate_rows_when_enabled(
    tmp_path,
):
    request = create_duplicate_request(
        tmp_path,
...
def test_prepare_keeps_duplicate_rows_when_disabled(
    tmp_path,
):
    request = create_duplicate_request(
        tmp_path,
